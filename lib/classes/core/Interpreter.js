...
class Interpreter {
    async parse(code, d) {
        ...
        d.compiled = compiled;
        let result = compiled.result.map(x => x.value), parsedFunctions = [];
        for (let index = 0; index < compiled.functions.length; index++) {
            ...
            if (!d.functions.has(token.name)) {
                if (token.fields.length < 1) {
                    ...
                    parsedFunctions.push(token.over);
                continue;
            }
            const params = [], func = d.functions.get(token.name);
            ...
            if (token.fields.length > 0) {
                for (let i = 0; i < token.fields.length; i++) {
                    ...
                    const field = token.fields[i], specification = func.parameters?.[i], dontParse = specification && 'parse' in specification && specification.parse === false, dontUnescape = specification && 'unescape' in specification && specification.unescape === false, parsed = dontParse ? field.value : (await this.parse(field.value, data)).code;
